...
1. 基于LLM+向量库的文档对话 基础面
    1. LLMs 存在模型幻觉问题，请问如何处理？
        ...
        模型幻觉（Model Hallucination）是指深度学习模型在训练和测试数据上表现良好，但在新的、未见过的数据上表现较差的现象。这通常是因为模型过度拟合了训练数据中的噪声或特定特征，导致在新的数据上无法很好地泛化。为了解决这个问题，可以采取以下策略：
        
        1. 重新采样数据：对训练数据进行重采样，以消除噪声或平衡类别分布。这可以通过过采样（增加少数类样本）或欠采样（减少多数类样本）来实现。
        2. 数据增强：通过对训练数据进行变换（如旋转、缩放、翻转等），生成更多的训练样本。这有助于模型学习到更多的特征，提高泛化能力。
        3. 使用预训练模型：利用在大量数据上预训练的模型进行迁移学习，以减少过拟合的风险。预训练模型已经学到了一些通用的特征，可以帮助模型在新数据上更好地泛化。
        4. 正则化：在损失函数中添加正则化项（如L1或L2正则化），以惩罚过大的权重值。这有助于防止模型过度拟合训练数据。
        5. Dropout：在训练过程中随机丢弃一部分神经元，以减少模型对单个特征的依赖。这有助于提高模型的泛化能力。
        6. 早停法（Early Stopping）：在验证集上监控模型性能，当性能不再提高时，停止训练。这有助于防止模型过度拟合训练数据。
        7. 集成学习：训练多个模型，并将它们的预测结果进行组合。这可以降低单个模型的过拟合风险，提高整体性能。
        8. 交叉验证：将训练数据分成K个子集，轮流将其中一个子集作为验证集，其余子集作为训练集。这有助于评估模型在不同数据子集上的性能，从而选择最佳模型。
        9. 超参数调优：使用网格搜索、随机搜索或贝叶斯优化等方法，寻找最佳的超参数组合，以提高模型的泛化能力。
        </aside>
        
    2. 基于LLM+向量库的文档对话 思路是怎么样？
        
        <aside>
        💡 **基于大语言模型和向量库的文档对话 其实现思路是怎么样的？**
    ...
    4. 基于LLM+向量库的文档对话 prompt 模板 如何构建？
        ...
        <aside>
        💡 **基于大语言模型和向量库的文档对话 其prompt 模板 如何构建？**
...
2. 基于LLM+向量库的文档对话 优化面
    1. 痛点1：文档切分粒度不好把控，既担心噪声太多又担心语义信息丢失
        ...
        <aside>
        💡 **在基于大语言模型和向量库的文档对话中，文档切分粒度不好把控，既担心噪声太多又担心语义信息丢失，如何解决？**
    ...
    5. 痛点5：如何让LLM基于query和context得到高质量的response
        ...
        <aside>
        💡 **如何让LLM基于query和context得到高质量的response?**
